...
| vLLM streaming tool-call issues | 已报告首个工具分片缺少 `type:function`，以及流模式把工具调用作为原始标签文本返回 | 高：提供真实缺陷类别和种子轨迹 | 单项目 issue 和回归测试，没有统一生成方法、跨客户端影响分析及跨版本实证 | [#16340](https://github.com/vllm-project/vllm/issues/16340), [#31871](https://github.com/vllm-project/vllm/issues/31871) |
| SGLang streaming compatibility issues | 并行调用缺少后续 call ID、重复同名工具共享 index 并被客户端合并 | 高：提供真实缺陷类别和种子轨迹 | 单项目、单场景修复；CompatFlow 应发现未进入其回归集的新组合并测量真实客户端后果 | [#7048](https://github.com/sgl-project/sglang/issues/7048), [#25073](https://github.com/sgl-project/sglang/issues/25073) |
